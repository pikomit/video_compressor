...
public struct VideoCompressor {
	...
	private let MIN_BITRATE = Float(2_000_000)
	...
	private func getBitrate(bitrate: Float, quality: VideoQuality) -> Int {
		if quality == .very_low {
			return Int(bitrate * 0.15)
		} else if quality == .low {
			return Int(bitrate * 0.30)
		} else if quality == .medium {
			return Int(bitrate * 0.45)
		} else if quality == .high {
			return Int(bitrate * 0.60)
		} else if quality == .very_high {
			return Int(bitrate * 0.75)
		} else {
			return Int(bitrate * 0.45)
		}
	}
